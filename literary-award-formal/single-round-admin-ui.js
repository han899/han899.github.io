...
(function(){
  ...
  function wire(form,edit=false){if(!form||form.dataset.singleRound==='1')return;form.dataset.singleRound='1';
    ...
    const wrap=document.createElement('div');wrap.className='single-round-picker';wrap.innerHTML='<label class="label">指定審查輪次</label><select class="select" data-round-choice><option value="">請選擇</option><option value="1">第一輪審查</option><option value="2">第二輪評分</option></select><p class="muted tiny">每位評審一次只指定一個審查輪次；需要更換時可由管理員編輯。</p>';
    ...
    const d1=form.elements.d1,d2=form.elements.d2;
    function sync(){const judge=role.value==='judge',v=sel.value;if(judge){r1.checked=v==='1';r2.checked=v==='2'}else{r1.checked=false;r2.checked=false}wrap.style.display=judge?'block':'none';sel.disabled=!judge;sel.required=judge;if(d1){d1.disabled=!judge||v!=='1';d1.closest('div').style.display=judge&&v==='1'?'block':'none'}if(d2){d2.disabled=!judge||v!=='2';d2.closest('div').style.display=judge&&v==='2'?'block':'none'}const sp=d1?.closest('.split');if(sp)sp.style.gridTemplateColumns='1fr'}
  ...
  function legacyNotice(){const main=document.querySelector('.main');if(!main||document.getElementById('legacyRoundNotice'))return;const cells=[...main.querySelectorAll('td')];if(!cells.some(td=>td.textContent.includes('兩輪')) )return;const sec=main.querySelector('.section');if(!sec)return;const n=document.createElement('div');n.id='legacyRoundNotice';n.className='security-note';n.innerHTML='<b>有既有評審仍是兩輪設定。</b><br>新版規則為單一輪次。請按該評審的「編輯」，由管理員明確指定第一輪或第二輪；系統不會自行猜測。';sec.querySelector('p')?.after(n)}
  function enhance(){if(routePage()!=='people')return;wire(document.getElementById('invite2'),false);wire(document.getElementById('em2'),true);legacyNotice()}
